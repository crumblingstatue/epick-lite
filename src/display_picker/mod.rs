...
pub fn init_display_picker() -> Option<Rc<X11DisplayPicker>> {
    X11DisplayPicker::new()
        .ok()
        .map(|conn| Rc::new(conn) as Rc<X11DisplayPicker>)
}

#[derive(Debug)]
pub struct X11DisplayPicker {
    conn: RustConnection,
    screen_num: usize,
}

impl X11DisplayPicker {
